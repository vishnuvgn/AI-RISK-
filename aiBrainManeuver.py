...
# this file is the ai who thinks about placement, attacking, and manuevering
...
def aiPlay(app):
    aiPlaySTEP1(app)
    aiPlaySTEP2(app)
    aiPlaySTEP3(app) # <-- new addition, in prior versions, ai didn't manuever
    ...
    for neighbor in worldMap[country.name]:
        if (neighbor.troopGeneral.name != app.currentPlayer.name):
            if(neighbor.troopCount > 1): # weeds out enemies who cannot attack
                # the number of troops that can actually fight. minus 1 because 
                # one troop always has to stay in that territory when attacking
                availableEnemies = neighbor.troopCount - 1
                dSum += availableEnemies
    return dSum
...
def aiPlaySTEP1(app):
    ...
    app.step2Now = True
    app.step3Now = False
    app.finishedRequired = True # all required thing (placement of troops) are done
...
# finds the strongest country ruled by currentPlayer.
# alreadyChecked is a set of countries that have already been checked and ruled
def findStrongest(app, alreadyChecked):
    allCountries = app.currentPlayer.territories # all of teh countries ruled by current player

    bestFromCountry = None
    bestFromCount = -1
    for country in allCountries:
        if(checkNeighbors(app, country) == False): # indicates that it is not on the border
            if(country not in alreadyChecked and country.troopCount > bestFromCount ):
                bestFromCountry = country
                bestFromCount = country.troopCount
    # returns the strongest country and the number of troos occupying the region currently
    return (bestFromCountry, bestFromCount)

# maneuvering for defense only
def aiPlaySTEP3(app):
    ...
    countryAndDeficit.sort(reverse=True) # list of the weakest countries ruled by currentPlayer
    print(f'countryDeficit = {countryAndDeficit}')
    ############

    # i dont use countryName, just there so the tuple doesn't complain
    for (dSum, countryName, country) in countryAndDeficit: # loop goes through the weak countries
        # first in countryAndDeficit is weakest
        alreadyChecked = set()

        goneThroughStrongs = False
        while(goneThroughStrongs == False): # this loop goes through the strong countries

            (fromCountry, fromCount) = findStrongest(app, alreadyChecked)
            if(fromCountry == None): # indicates that there are no strong countries
                goneThroughStrongs = True
                breadCrumbs = []
                break

            breadCrumbs = []
            if(isThereAPath(worldMap, fromCountry.name, country, breadCrumbs, app.currentPlayer) == True):
                # the maxTroops to move is two thirds of the troop available to be moved from the strongest
                maxTroopsToMove = int(round((2/3) * (fromCountry.troopCount - 1), 0)) # to 0 decimal places (rounds to nearest int)
                # no need for any maneuvering
                if(dSum == 0 or dSum - country.troopCount <= 0):
                    goneThroughStrongs = True
                    breadCrumbs = []
                    break
                
                # checks if there is any point in maneuvering to that country
                troopsMoving = (dSum - country.troopCount)
                if(troopsMoving <= maxTroopsToMove):
                    country.troopCount += troopsMoving
                    fromCountry.troopCount -= troopsMoving
                    print(f'maneuvering {troopsMoving} troops from {fromCountry.name} to {country.name}')
                    goneThroughStrongs = True
                else:
                    alreadyChecked.add(fromCountry)
            else:
                alreadyChecked.add(fromCountry)

            breadCrumbs = [] # reset breadCrumbs for the recursive formula
